...
def clip_video(url):
    ...
    try:
        ...
        for i, clip in enumerate(clip_data):
            ...
            ffmpeg.input(video_file, ss=clip['start_time'], t=clip['duration']).output(
                clip_id + ".mp4",
                avoid_negative_ts='make_zero',
                vf=f"scale=-1:1280,crop=720:1280,subtitles={sub_path}",
                vcodec="libx264",
                acodec="copy",
                crf=18,
            ).run()
...
def format_subtitle_dialogue(start, duration, subs_list):
    ...
    while index < len(key_list) - 1 and time_to_ms(key_list[index]) < time_to_ms(end):
        ...
        dialogue += f"Dialogue: 0,{ms_to_time(time_to_ms(temp_start) - time_to_ms(start))},{ms_to_time(time_to_ms(temp_end) - time_to_ms(start))},Default,,0,0,0,,{temp_content}".replace('\n', '\\N') + "\n"
        index += 1
    return dialogue
